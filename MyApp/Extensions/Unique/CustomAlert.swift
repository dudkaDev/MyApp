...
class CustomAlert {
    ...
    func presentCustomAlert(viewController: UIViewController, repsOrTimer: String, completion: @escaping(String, String) -> Void) {
        ...
        let repsOrTimer = UILabel(text: repsOrTimer)
        repsOrTimer.translatesAutoresizingMaskIntoConstraints = true
        repsOrTimer.frame = CGRect(
            x: 30,
            y: setsTextField.frame.maxY + 3,
            width: alertView.frame.width - 60,
            height: 20
        )
        alertView.addSubview(repsOrTimer)
        
        repsTextField.frame = CGRect(
            x: 20,
            y: repsOrTimer.frame.maxY,
            width: alertView.frame.width - 40,
            height: 30
        )
    ...
    @objc private func okButtonTapped() {
        ...
        guard let setNumber = setsTextField.text, let repsNumber = repsTextField.text else { return }
